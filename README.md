...
`connectOptions.sessionId`; the rest is your normal model UI. See
...
2. Creates the Reactor session lazily on `claim()` (`POST /sessions`), then sends
   `session_ready { sessionId }` so the client attaches via `connect({ sessionId })`.
   Creating nothing during grace means an abandoned admission never orphans a GPU
   session.
...
The Reactor session is created on `claim()`, not at admission. When the first
member of a slot claims, the server calls `POST /sessions`, stores the id on the
slot, and sends `session_ready { sessionId }`. Other members of the same slot
reuse that id when they claim — so an admitted user who never claims (grace
timeout, tab close) frees their slot with no GPU session ever created.
...
created (and `RQ_STOP_SESSIONS` is on) and removes the slot.
...
still rolling out — the default `1` matches current behavior.
...
The queue client sets `sessionId` from the `admitted` WebSocket message.
