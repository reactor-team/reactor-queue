...
// without the `--localstorage-file` flag: it resolves to an object whose
// Storage methods are missing, and on modern Node it shadows the working
// localStorage that the jsdom test environment would otherwise provide. That
// makes the browser-side suites fail with "localStorage.clear is not a
// function" - the same Node 24+ quirk that src/client.ts guards against at
// runtime.
//
...
// where jsdom's real localStorage is kept; contributors on Node 22/24/25 get a
// working polyfill so `pnpm test` is green everywhere.
